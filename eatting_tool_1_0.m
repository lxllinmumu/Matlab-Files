function last_supper = eatting_tool_1_0()
clc;clear;close all;
%% 改进想法及现存问题
%截止时间：eatting_tool 1.0版到2018.11.22基本完成

%用途：主要解决个人（209寝室）对饮食的选择困难问题

%功能：菜单选择（需个人编写xls或xlsx菜单）
%      音乐播放
%      菜品随机选择（支持类型筛选）
%      时间显示

% 1.如何将提示信息显示在带有鼠标事件的背景图上（如：喇叭）;
...
hAxes_BGM = axes('parent',h_figure);
set(hAxes_BGM,...
    'xtick',[],'ytick',[],...
    'units','pixels',...
    'position',[310 254 35 28]);
h_BGM = imshow(BGM_JPG);
set(h_BGM,'ButtonDownFcn',@BGM_CallbackFcn);
...
Text_UI3 = uicontrol('position',[120 150 160 30],...
                     ...
                     'fontweight','bold',...
                     'visible','on'); 
BGM_UI = uicontrol('position',[310 280 35 15],...
                   'style','text','string','音乐',...
...
List_Over_Text = {'主人用餐偷税~嘤嘤',...
                  '主人！这个好吃~~',...
                  '主人！最喜欢了~'};
Popupmenu_UI1 = uicontrol('position',[55 125 60 55],...
